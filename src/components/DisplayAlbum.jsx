  ...
  return (
    ...
    <div className="mt-10 flex gap-8 flex-col md:flex-row md:items-end">
        ...
        <div className="flex flex-col">
            <p>Playlist</p>
            <h2 className="text-5xl font-bold mb:4 md:text-6xl">{albumData.name}</h2><br></br>
            <h4>{albumData.desc}</h4>
            <p className="mt-1">
                <img src={assets.spotify_logo} alt="" className="inline-block w-5 gap-1" />
                <b> Spotify </b>
                • 1,323,154 likes   
                • <b>48 songs, </b>
                about 1hr 28 min       
    ...
    <div className="grid grid-cols-3 sm:grid-cols-4 mt-10 mb-4 pl-2 text-[#a7a7a7]">
        ...
        <p className="hidden sm:block text-center">Duration</p>
...

         {
            songsData.map((item,index)=>(
                <div onClick={()=>playWithId(item.id)} key={index} className="grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer">
                  ...
                  <p className="text-[15px] hidden sm:block text-center">
                    {item.duration}
                  </p>        
            </div>
        ))}
